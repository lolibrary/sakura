...
In addition, the codebase currently relies on one non-free component: [Laravel Nova](https://nova.laravel.com/). It is not distributed with the project, and anyone wishing to reuse the codebase will need to provide their own copy. Volunteers working on the main Lolibrary.org codebase who need copies for local development work only should speak to one of the head devs.
